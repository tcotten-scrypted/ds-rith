...
export default async function update(state) {
    ...
    const discoCenter = state.world?.buildings.find(
        (b) => b.kind?.name?.value == "Disco Center",
    );
    ...
    if (mobileUnit){
        const buildingTileCoords = getTileCoords(discoCenter?.location?.tile?.coords);
        ...
        if (disco) {
            getTilesInRange(discoCenter, TILE_COLOUR_DISTANCE).forEach((t) => {
                if (t !== unitTileId) {
                    map.push(
                        {
                            type: "tile",
